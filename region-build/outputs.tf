locals {
  ...
  # To avoid possible issues when running terraform destroy
  controller_public_ip = length(aws_eip.controller_eip) > 0 ? aws_eip.controller_eip[0].public_ip : ""
  copilot_public_ip    = length(aws_eip.copilot_eip) > 0 ? aws_eip.copilot_eip[0].public_ip : ""
}
