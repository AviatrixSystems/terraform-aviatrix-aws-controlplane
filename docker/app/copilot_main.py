...
def controller_copilot_setup(api, event):
    ...
    api.enable_copilot_association(
        event["copilot_info"]["private_ip"], event["copilot_info"]["public_ip"]
    )
    ...
    api.enable_copilot_sg(
        event["primary_account_name"],
        "1",
        event["region"],
...
def get_vm_password(pass_type="copilot"):
    if (
        pass_type == "copilot"
        and os.environ.get("COP_EMAIL", "") != ""
        and os.environ.get("COP_USERNAME", "") != ""
    ):
...
def get_restore_region():
    ...
    if os.environ.get("INTER_REGION", "") == "True" and os.environ.get("PRIV_IP", ""):
        print(
            f"inter-region HA in current region '{os.environ.get('SQS_QUEUE_REGION', '')}'"
        )
        if get_instance_recent_restart("controller"):
            restore_region = os.environ.get("DR_REGION", "")
            print(
                f"Controller was also restarted recently, so we will assume regional failure"
            )
            print(f"restore controller and copilot to dr region'{restore_region}'")
        else:
            restore_region = os.environ.get("SQS_QUEUE_REGION", "")
            print(
                f"HA event in an inter-region deployment, but the controller was not restarted recently. We will assume that only the CoPilot VM failed"
            )
            print(
                f"restore copilot in current region because assuming controller did not fail in inter-region HA: '{restore_region}'"
            )
    else:
        restore_region = os.environ.get("SQS_QUEUE_REGION", "")
        if os.environ.get("INTER_REGION", "") == "True":
            ...
            print(
                f"current region '{os.environ.get('SQS_QUEUE_REGION', '')}' is inter-region primary '{os.environ.get('ACTIVE_REGION', '')}'"
            )
        else:
            print(
                f"intra-region init/HA - create/restore in current region: '{restore_region}'"
            )
...
def modify_sg_rules(ec2_client, operation, security_group_id, sg_rule) -> None:
    try:
        ...
        modify_rule = {
            ...
            "IpRanges": [{"CidrIp": cidr} for cidr in sg_rule["cidr_list"]],
        }
        ...
        data = fn(GroupId=security_group_id, IpPermissions=[modify_rule])
        print(f"Rules successfully modified: {data}")
        return security_group_id
    except Exception as err:  # pylint: disable=broad-except
        print(f"Modifying SG rules error: {err}")
        print(
            f"operation: {operation} - security_group_id: {security_group_id} - sg_rule: {sg_rule}"
        )
    ...
    try:
        ...
        if "SecurityGroups" in response:
            ...
            for each_rule in all_rules:
                if (
                    each_rule["IpProtocol"] == check_rule["IpProtocol"]
                    and each_rule["FromPort"] == check_rule["FromPort"]
                ):
...
def clear_security_group_rules(ec2_client, security_group_id):
    ...
    if "SecurityGroups" in response:
        ...
        for rule in security_group["IpPermissions"]:
            for ip_range in rule["IpRanges"]:
                if ip_range.get("Description"):
                    if (
                        "CoPilot private IP" in ip_range["Description"]
                        or "CoPilot public IP" in ip_range["Description"]
                    ):
                        delete_rule = {
        ...
        print(
            f"Successfully deleted CoPilot public and private IP rules in SG {security_group_id}"
        )
    else:
...
def handle_copilot_ha():
    ...
    if (
        inter_region
        and copilot_init
        and os.environ.get("SQS_QUEUE_REGION", "")
        == os.environ.get("STANDBY_REGION", "")
    ):
        print(
            f"Not initializing copilot in the standby region '{os.environ.get('SQS_QUEUE_REGION', '')}' in inter-region init"
        )
        return
    ...
    copilot_instanceobj = aws_utils.get_ec2_instance(
        restore_client, copilot_instance_name, ""
    )
    ...
    controller_instanceobj = aws_utils.get_ec2_instance(
        restore_client, controller_instance_name, ""
    )
    if controller_instanceobj == {}:
        print(
            f"Unable to find controller instance '{controller_instance_name}' - Abort HA"
        )
        return False
    print(f"controller_instanceobj: {controller_instanceobj}")

    instance_public_ips = get_controller_copilot_public_ips(
        controller_instanceobj, copilot_instanceobj
    )
    if os.environ.get("COP_AUTH_IP", "") == "private":
    ...
    if os.environ.get("COP_DEPLOYMENT", "") == "fault-tolerant":
        ...
        for inst in data_node_details:
            data_node_instanceobj = aws_utils.get_ec2_instance(
                restore_client, inst["instance_name"], ""
            )
            if data_node_instanceobj == {}:
                print(
                    f"Unable to find data node instance {inst['instance_name']} - Skip instance"
                )
                break
            copilot_data_node_public_ips.append(
                data_node_instanceobj["PublicIpAddress"]
            )
            copilot_data_node_private_ips.append(
                data_node_instanceobj["PrivateIpAddress"]
            )
            ...
            copilot_data_node_sg_names.append(
                data_node_instanceobj["SecurityGroups"][0]["GroupName"]
            )
            copilot_data_node_sg_ids.append(
                data_node_instanceobj["SecurityGroups"][0]["GroupId"]
            )
    ...
    copilot_event = {
        ...
        "copilot_type": os.environ.get(
            "COP_DEPLOYMENT", ""
        ),  # values should be "simple" or "fault-tolerant"
        "copilot_custom_user": copilot_user_info[
            "custom_user"
        ],  # true/false based on copilot service account
    ...
    if not copilot_init:
        ...
        print(
            f"copilot ha event - clear rules in copilot main sg: {copilot_instanceobj['SecurityGroups'][0]['GroupId']}"
        )
        clear_security_group_rules(
            restore_client, copilot_instanceobj["SecurityGroups"][0]["GroupId"]
        )
        if os.environ.get("COP_DEPLOYMENT", "") == "fault-tolerant":
    ...
    copilot_tmp_sg = manage_tmp_access(
        restore_client, copilot_instanceobj["SecurityGroups"][0]["GroupId"], "add_rule"
    )
...
def handle_event(event):
    ...
    if event["copilot_type"] == "simple":
        ...
        print(
            f"Adding Controller auth IP '{event['auth_ip']}' to CoPilot SG '{event['copilot_info']['sg_id']}'"
        )
    ...
    copilot_api = single_cplt.CoPilotAPI(
        copilot_ip=event["copilot_info"]["public_ip"], cid=api._cid
    )

    # set the new copilot to use the controller to verify logins
    print(f"Set controller auth IP '{event['auth_ip']}' on CoPilot")
    set_controller_ip_resp = copilot_api.retry_set_controller_ip(
        event["auth_ip"],
    ...
    if event["copilot_init"]:
        ...
        if event["copilot_type"] == "simple":
            ...
            copilot_api.wait_copilot_init_complete(
                event["copilot_type"], copilot_init_config
            )
        else:
        ...
        if config.get("singleCopilot"):
            config["singleCopilot"]["copilotConfigFiles"]["db.json"]["config"][
                "controllerIp"
            ] = event["auth_ip"]
        elif config.get("mainCopilot"):
            config["mainCopilot"]["copilotConfigFiles"]["db.json"]["config"][
                "controllerIp"
            ] = event["auth_ip"]
    ...
    # 3. login via session
    response = copilot_api.session_login(
        username=event["copilot_info"]["user_info"]["username"],
        password=event["copilot_info"]["user_info"]["password"],
    )
