...
def ecs_handler():
    ...
    # If HA_DISTRIBUTION is inter-region-v2, the code only needs to handle the Controller initialization.
    # Failover will happen based on the message generated by the failed health check and not the ASG message.
    elif (
        os.environ.get("INTER_REGION") == "True"
        and msg_asg == os.environ.get("CTRL_ASG")
        and os.environ.get("PRIV_IP")
        and os.environ.get("HA_DISTRIBUTION") != "inter-region-v2"
    ):
        pri_region = region
...
def update_env_dict(ecs_client, replace_dict={}):
    ...
    env_dict = {
        ...
        "S3_BUCKET_REGION2": os.environ.get("S3_BUCKET_REGION2", ""),
        "ENABLE_SECONDARY_BACKUP": os.environ.get("ENABLE_SECONDARY_BACKUP"),
