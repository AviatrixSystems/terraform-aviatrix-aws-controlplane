...
| avx_copilot_password             |                                         | The CoPilot service account password. WARNING: The password will be visible in the container's environment variables. See above note for more information.                                                                                |
| avx_copilot_password_ssm_path    | /aviatrix/copilot/password              | The path to the CoPilot password. Only applicable if `avx_copilot_password` is not specified.The value should be set/updated in `region` and also in `dr_region` if deploying `inter-region` or `inter-region-v2`.                        |
...
| avx_customer_id_ssm_path         | /aviatrix/controller/customer_id        | The path to the Aviatrix customer ID. Only applicable if `avx_customer_id` is not specified. The value should be set/updated in `region` and also in `dr_region` if deploying `inter-region` or `inter-region-v2`.                        |
...
| avx_password_ssm_path            | /aviatrix/controller/password           | The path to the Aviatrix password. Only applicable if `avx_password` is not specified. The value should be set/updated in `region` and also in `dr_region` if deploying `inter-region` or `inter-region-v2`.                              |
...
| controller_version               | latest                                  | The initial version of the Controller at launch                                                                                                                                                                                           |
...
| dr_vpc_cidr                      | 10.0.1.0/24                             | The CIDR for the VPC to create for the DR Controller. Only applicable if `ha_distribution` is `inter-region` and `use_existing_vpc` is `false`.                                                                                           |
...
| ha_distribution                  | basic                                   | Desired Controller high availability distribution. Valid values are `basic`, `single-az`, `inter-az`, `inter-region` and `inter-region-v2`.                                                                                               |
| healthcheck_dr_private_rt_ids    | []                                      | The list of private route table IDs in the DR region to add a route to the primary VPC via a VPC peering connection. Only applicable if `ha_distribution` is `inter-region-v2` and `use_existing_vpc` is `true`.                          |
| healthcheck_dr_public_rt_ids     | []                                      | The list of public route table IDs in the DR region to add a route to the primary VPC via a VPC peering connection. Only applicable if `ha_distribution` is `inter-region-v2` and `use_existing_vpc` is `true`.                           |
| healthcheck_dr_subnet_ids        | []                                      | The list of private subnets to use for the healthcheck Lambda function in the DR region. Only applicable if `ha_distribution` is `inter-region-v2` and `use_existing_vpc` is `true`.                                                      |
| healthcheck_interval             | 5                                       | The number of minutes to wait between healthchecks                                                                                                                                                                                        |
| healthcheck_policy_name          | aviatrix-healthcheck-policy             | The name of the healthcheck Lambda policy                                                                                                                                                                                                 |
| healthcheck_private_rt_ids       | []                                      | The list of private route table IDs in the primary region to add a route to the DR VPC via a VPC peering connection. Only applicable if `ha_distribution` is `inter-region-v2` and `use_existing_vpc` is `true`.                          |
| healthcheck_public_rt_ids        | []                                      | The list of public route table IDs in the primary region to add a route to the DR VPC via a VPC peering connection. Only applicable if `ha_distribution` is `inter-region-v2` and `use_existing_vpc` is `true`.                           |
| healthcheck_role_name            | aviatrix-role-healthcheck               | The name of the healthcheck Lambda role                                                                                                                                                                                                   |
| healthcheck_subnet_ids           | []                                      | The list of private subnets to use for the healthcheck Lambda in the primary region. Only applicable if `ha_distribution` is `inter-region-v2` and `use_existing_vpc` is `true`.                                                          |
| incoming_ssl_cidr                |                                         | Incoming CIDR for security group used by Controller                                                                                                                                                                                       |
| instance_type                    | t3.large                                | Controller instance size                                                                                                                                                                                                                  |
...
| monitoring                       | false                                   | Whether detailed monitoring is enabled. Applies to both the Controller and CoPilot.                                                                                                                                                       |
...
| s3_backup_bucket2                | aviatrix-ha-                            | Second S3 bucket for Controller DB backup. Only applicable if `enable_secondary_backup` is `true`.                                                                                                                                        |
| s3_backup_region                 | us-east-1                               | The region the S3 backup bucket is in                                                                                                                                                                                                     |
| s3_backup_region2                | us-east-2                               | AWS region of second S3 backup bucket. Only applicable if `enable_secondary_backup` is `true`.                                                                                                                                            |
...
| use_existing_copilot_eip         | false                                   | Set to `true` to use the EIP(s) specified by `existing_copilot_eip` (and `existing_copilot_dr_eip`) for the CoPilot(s) rather than allocating new EIPs                                                                                    |
| use_existing_eip                 | false                                   | Set to `true` to use the EIP(s) specified by `existing_eip` (and `existing_dr_eip`) for the Controller(s) rather than allocating new EIPs                                                                                                 |
